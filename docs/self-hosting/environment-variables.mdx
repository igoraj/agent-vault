...
| `AGENT_VAULT_NETWORK_ALLOWLIST` | (unset) | Comma-separated CIDRs or bare IPs to allow when `AGENT_VAULT_ALLOW_PRIVATE_RANGES=false` (e.g. `10.163.0.0/16,192.168.1.1`). Bare IPv4 expands to `/32`, bare IPv6 to `/128`. IMDS endpoints stay blocked. Has no effect when `ALLOW_PRIVATE_RANGES=true`. |
